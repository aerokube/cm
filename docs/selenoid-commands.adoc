...
To simplify this process you can copy and run small script:

.On Linux
[source,bash]
----
curl -L -o cm `curl -s https://api.github.com/repos/aerokube/cm/releases/latest | grep "browser_download_url" | grep linux | cut -d : -f 2,3 | tr -d \"` \
&& chmod +x ./cm \
...
[source,bash]
----
curl -L -o cm `curl -s https://api.github.com/repos/aerokube/cm/releases/latest | grep "browser_download_url" | grep darwin | cut -d : -f 2,3 | tr -d \"` \
&& chmod +x ./cm \
&& ./cm selenoid start --vnc
----
